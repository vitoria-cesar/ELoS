...
const ambientLight = new THREE.HemisphereLight('white','darkslategrey',0.5) //LUZ DO PLANO

const mainLight = new THREE.DirectionalLight('white',0.7) //AMBIENTE PRINCIPAL / UZ DIRECIONAL
mainLight.position.set(2,1,1)

const controls = new OrbitControls(camera, renderer.domElement)

const gridMapHelper = new GridMapHelper() //CRIA AS DIVISÕES DO PLANO 

const plane = gridMapHelper.createGridPlane() //CRIA O PLANO

var actorModelPath = new URL('../../../assets/models/eve.glb',import.meta.url).toString() // CRIA E POSICIONA O ROBÔ
...
const objective = new THREE.Object3D() // CRIA E POSICIONA O OBSTÁCULO
...
function checkCollision(object1,object2) //TESTA SE O ROBO E O CRISTAL ESTÃO NA MESMA COORDENADA
{
...
//POSIÇÃO ORIGINAL SOMENTE DO ROBÔ, POIS ELE É O ÚNICO QUE 'ANDA'
function resetLevel()
{
    actor.position.set(gridMapHelper.getGlobalXPositionFromCoord(0),1.0,gridMapHelper.getGlobalZPositionFromCoord(5))
    actor.rotation.set(0,degreeToRadians(90),0)
    actor.getObjectByName('eve').rotation.set(0,0,0)
    objective.visible = true //CRISTAL FICA VISÍVEL
}
...
advanceBtn.addEventListener('click',function(e){
    e.preventDefault()
    setTimeForNextPhase('/level1/phase2/',getTotalTime(sceneProperties.phaseTimer.getElapsedTime()))
